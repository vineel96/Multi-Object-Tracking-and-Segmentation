...
sys.path.insert(1, '/home/mot_1/SiamMask')
...
from utils.bbox_helper import cxy_wh_2_rect, get_axis_aligned_bbox
...
from utils.pyvotkit.region import vot_float2str, vot_overlap
...
def get_subwindow_tracking(im, model_sz, avg_chans, out_mode='torch', targets=None, targetpos=None, s_z=None):
    im_sz = im.shape
    # print("SubWIndow")
    ...
    for target in targets:
        ...
        # sz = s_zz
        c = (sz + 1) / 2
        context_xmin = round(target["target_pos"][0] - c)
        context_xmax = context_xmin + sz - 1
        context_ymin = round(target["target_pos"][1] - c)
...
def siamese_init(im, model, hp=None, device='cpu', targets=None, detector=None):
    ...
    targets = get_subwindow_tracking(im, p.exemplar_size, avg_chans, targets=targets, )
