...
def get_subwindow_tracking(im,state,model_sz, avg_chans, out_mode='torch'):
    im_sz = im.shape
    for index in range(len(state)):
        if isinstance(state[index]["target_pos"], float):
            pos = [state[index]["target_pos"],state[index]["target_pos"]]
        pos=state[index]["target_pos"]
        sz = state[index]["s_z"]
        ...
        state[index]["im_patch"]=im_to_torch(im_patch) if out_mode in 'torch' else im_patch
    return state
...
def siamese_init(im, state, model, hp=None, device='cpu'):
    #print("frame shape: ",im.shape)
    for index in range(len(state)):
        state[index]['im_h'] = im.shape[0]
        state[index]['im_w'] = im.shape[1]
    p = TrackerConfig()
    ...
    for index in range(len(state)):
        wc_z = state[index]["target_sz"][0] + p.context_amount * sum(state[index]["target_sz"])
        hc_z = state[index]["target_sz"][1] + p.context_amount * sum(state[index]["target_sz"])
        state[index]["s_z"] = round(np.sqrt(wc_z * hc_z))
    # initialize the exemplar
    state = get_subwindow_tracking(im, state, p.exemplar_size, avg_chans)

    for index in range(len(state)):
        z = Variable(state[index]["im_patch"].unsqueeze(0))
        #print("shape of object :",index, z.shape)
        state[index]["zf"] = net.template(z.to(device))
        del state[index]["im_patch"]
    ...
    for index in range(len(state)):
        state[index]['p'] = p
        state[index]['net'] = net
        state[index]['avg_chans'] = avg_chans
        state[index]['window'] = window
    return state

def siamese_track(f, state, im, mask_enable=False, refine_enable=False, device='cpu', debug=False):

    if len(state) > 0 : # atleast one object state should be present to track #recent change
        p = state[0]['p']
        net = state[0]['net']
        avg_chans = state[0]['avg_chans']
        window = state[0]['window']

        # removing states of objects that go out of frame
        ...
        while change:
            ...
            for index in range(length):
                x_pos = state[index]['target_pos'][0]
                y_pos = state[index]['target_pos'][1]
                pos_width = state[index]['target_sz'][0]
                pos_height = state[index]['target_sz'][1]
                print("x pos,width:", x_pos, image_width)
                if x_pos + 700 >= image_width and f >= 8:  # included frame number also. initial -50 #recent change
                    print("inside")
                    del state[index]
                    change = 1
                    break
                if x_pos - 160 <= 0 and f >= 5:
                    del state[index]
                    change = 1
                    break
                if y_pos - 150 <= 0:
        ...
        for index in range(len(state)):
            target_pos = state[index]['target_pos']
            target_sz = state[index]['target_sz']
            ...
            s_x = s_x + 2 * pad
            state[index]["s_z"] = s_x
            state[index]["scale_x"] = scale_x
            state[index]["crop_box"] = [target_pos[0] - round(s_x) / 2, target_pos[1] - round(s_x) / 2, round(s_x),
                                        round(s_x)]

            if debug:
                im_debug = im.copy()
                crop_box_int = np.int0(state[index]["crop_box"])
                cv2.rectangle(im_debug, (crop_box_int[0], crop_box_int[1]),
                              (crop_box_int[0] + crop_box_int[2], crop_box_int[1] + crop_box_int[3]), (0, 0, 0), 2)
                cv2.imshow('search area', im_debug)
                cv2.waitKey(0)

        # extract scaled crops for search region x at previous target position
        state = get_subwindow_tracking(im, state, p.instance_size, avg_chans)
        list = []
        for index in range(len(state)):
            state[index]["x_crop"] = Variable(state[index]["im_patch"].unsqueeze(0))
            # print("x_crop size of object: ",index,state[index]["x_crop"].shape)
            state[index]["x_crop"] = state[index]["x_crop"].to(device)
            list.append({"x_crop": state[index]["x_crop"], "zf": state[index]["zf"]})

        if mask_enable:
            positions = net.track_mask(list)  # edit by GVA added state_index
        else:
            score, delta = net.track(x_crop.to(device))

        for index in range(len(positions)):
            delta = positions[index]["rpn_pred_loc"]
            score = positions[index]["rpn_pred_cls"]
            ...
            positions[index]["rpn_pred_loc"] = delta
            positions[index]["rpn_pred_cls"] = score
        ...
        count = 0
        for object, position in zip(state, positions):
            delta = position["rpn_pred_loc"]
            score = position["rpn_pred_cls"]
            crop_box = object["crop_box"]
            mask = position["rpn_pred_mask"]

            # size penalty
            target_sz_in_crop = object["target_sz"] * object["scale_x"]
            ...
            pred_in_crop = delta[:, best_pscore_id] / object["scale_x"]
            lr = penalty[best_pscore_id] * score[best_pscore_id] * p.lr  # lr for OTB

            res_x = pred_in_crop[0] + object["target_pos"][0]
            res_y = pred_in_crop[1] + object["target_pos"][1]

            res_w = object["target_sz"][0] * (1 - lr) + pred_in_crop[2] * lr
            res_h = object["target_sz"][1] * (1 - lr) + pred_in_crop[3] * lr

            object["target_pos"] = np.array([res_x, res_y])
            object["target_sz"] = np.array([res_w, res_h])
            ...
            if mask_enable:
                ...
                back_box = [-sub_box[0] * s, -sub_box[1] * s, state[0]['im_w'] * s, state[0]['im_h'] * s]
                mask_in_img = crop_back(mask, back_box, (state[0]['im_w'], state[0]['im_h']))
                ...
                if len(contours) != 0 and np.max(cnt_area) > 100:
                    ...
                    location = cxy_wh_2_rect(object["target_pos"], object["target_sz"])
                    rbox_in_img = np.array([[location[0], location[1]],
                                            [location[0] + location[2], location[1]],
                                            [location[0] + location[2], location[1] + location[3]],
                                            [location[0], location[1] + location[3]]])

            object["target_pos"][0] = max(0, min(state[0]['im_w'], object["target_pos"][0]))
            object["target_pos"][1] = max(0, min(state[0]['im_h'], object["target_pos"][1]))
            object["target_sz"][0] = max(10, min(state[0]['im_w'], object["target_sz"][0]))
            object["target_sz"][1] = max(10, min(state[0]['im_h'], object["target_sz"][1]))

            object['score'] = score[best_pscore_id]
            object['mask'] = mask_in_img if mask_enable else []
            object['ploygon'] = rbox_in_img if mask_enable else []
