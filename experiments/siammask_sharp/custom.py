...
#edit by GVA added list for zf and state_index
class Custom(SiamMask):
    ...
    def template(self, template):
        #edit by GVA
        #self.zf.append(self.features(template))
        import time
        s=time.time()
        feature=self.features(template)
        #print("Time in Init template:",time.time()-s)
        return feature
    ...
    def track_mask(self, list):
        ...
        for index in range(len(list)):
            self.feature, self.search = self.features.forward_all(list[index]["x_crop"])
            rpn_pred_cls, rpn_pred_loc = self.rpn(list[index]["zf"], self.search)
            self.corr_feature = self.mask_model.mask.forward_corr(list[index]["zf"], self.search)
            pred_mask = self.mask_model.mask.head(self.corr_feature)
            r.append({"rpn_pred_cls": rpn_pred_cls, "rpn_pred_loc": rpn_pred_loc, "rpn_pred_mask": pred_mask})
            self.feat.append(self.feature)
            self.corr_feat.append(self.corr_feature)
        #print("Tracking Time in track_mask: ",time.time()-start_time)
        return r

    def track_refine(self, pos,index):
        pred_mask = self.refine_model(self.feat[index], self.corr_feat[index], pos=pos, test=True)
        return pred_mask
